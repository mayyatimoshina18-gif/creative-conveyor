...
async function bootstrap() {
  try {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (req.method === "POST" && req.url === "/api/executors/update-profile") {
        let body = "";
        ...
        req.on("end", async () => {
          try {
            ...
            const specializations = Array.isArray(payload.specializations)
              ? payload.specializations.map(item => String(item).trim()).filter(Boolean)
              : [];
            const portfolio = payload.portfolio ? String(payload.portfolio).trim() : null;
            const paymentMethod = String(payload.paymentMethod || "").trim();
            const paymentDetailsValue = String(payload.paymentDetails || "").trim();
            const unavailableDays = Array.isArray(payload.unavailableDays)
              ? payload.unavailableDays.map(item => String(item).trim()).filter(Boolean)
              : [];
            const unavailableTime = payload.unavailableTime ? String(payload.unavailableTime).trim() : "";

            if (!fullName || !telegramContact || !specializations.length || !paymentMethod || !paymentDetailsValue) {
              sendJson(res, 400, { error: "Missing required fields" });
              return;
            }

            const sensitiveChanged =
              String(profile.fullName || "").trim() !== fullName ||
              String(profile.telegramContact || "").trim() !== telegramContact ||
              JSON.stringify(profile.specializations || []) !== JSON.stringify(specializations);
            ...
            sendJson(res, 200, {
              ok: true,
              requiresModeration: sensitiveChanged,
              executor: serializeExecutorProfile(profile)
            });
          } catch (error) {
            console.error("POST /api/executors/update-profile error:", error);
            sendJson(res, 500, { error: "Failed to update executor profile" });
          }
        });
